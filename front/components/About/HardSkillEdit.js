...
import { fetchUserHardSkill, putUserHardSkill } from "../../services/masterService";
import {
...
class HardSkillEdit extends React.Component {
    constructor(props) {
        super(props);
        this.state = {
            searchTxt: "",
    ...
    async _loadData() {
        if (this.props.anotherProfile) { 
            ...
            this.setState({ masterSoftSkill: res.data.payload.hardSkills });
        }
        else {
            await fetchUserHardSkill()
                .then((res) => {
    ...
    async _setscore(id, inputnumber) {
        await putUserHardSkill(id, inputnumber);
        this._loadData();
    }
...
export default withToastManager(HardSkillEdit)