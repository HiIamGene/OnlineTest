...
import PostTime from "./PostComponent/PostTime";
...
class Reactions extends Component {
    ...
    getInitialIconLike() {
        const { t } = this.props;
        const { myEmotion, emotionIcon } = this.state;
        let emotionTypeIcon = Object.keys(emotionIcon);
        let emotionTypeText;
        if (myEmotion === "" || myEmotion === "LIKE") {
            this.setState({
                myEmotionIcon: (
                    <i
                        className={
            ...
            for (let i = 0; i < emotionTypeIcon.length; i++) {
                for (let value in emotionIcon) {
                    if (myEmotion === value) {
                        this.setState({
                            myEmotionIcon: (
                                <img
                                    src={emotionIcon[value]}
                                    className="emotion-button-like"
                                />
                            )
                        });
                    }
    ...
    async postEmotion(like) {
        ...
        like = payload.post.myEmotion;
        const emotionTypeIcon = Object.keys(emotionIcon);
    ...
    async shareUpload(e) {
        ...
        // console.log(config)
        await axios
            .post(`${API_FEED_SHARE}/${this.props.post.id}`, { ...config })
    ...
    redirectToNextPage = postId => {
        window.open(`/postViewer?id=${postId}`, "_blank");
    };

    async onEmotionFromLikeButton() {
        const res = await axios.get(`${API_FEED_POST}/${this.props.post.id}`);
        const { data } = res;
        const { payload } = data;
        const currentEmotion =
            payload.post.myEmotion === "" ? "LIKE" : payload.post.myEmotion;
        this.postEmotion(currentEmotion);
    }
        ...
        return (
            <div className={"reaction"}>
                            ...
                            {this.state.isLikeLoading ? (
                                ...
                                <>
                                    ...
                                    <img
                                        className="like-icon"
                                        src={
                                            emotionsImages[2] !== undefined
                                                ? emotionsImages[2]
                                                : ""
                                        }
                                    />{" "}
                                    {this.state.emotionCount !== 0
                                        ? this.state.emotionCount
                                        : ""}
                                </>
                            )}
                        </div>
                ...
                <div
                    className={"panel"}
                    style={{
                        justifyContent: this.props.feed.isPostPopup
                            ? "space-around"
                            : "space-between"
                    }}
                >
                    <button
                        ...
                        onClick={() => this.onEmotionFromLikeButton()}
                    >
                        {myEmotionIcon} {t("POSTS.like")}
                    </button>
                    <div
                        id="like"
                        className={
                        ...
                        <div className="btn-container">
                            ...
                            <button className="alt-like-icon">
                                <img
                                    src={iconURL[5]}
                                    className="like-icon"
                                    onClick={() => this.onClickLike(5)}
                                />
                            </button>
                        </div>
                    </div>
                    {this.props.feed.isPostPopup ||
                    this.props.feed.isPostCommentPopup ||
                    this.props.feed.isPostImagePopup ? (
                        ""
                    ) : (
                        <button
                            className={"btn btn"}
                            onClick={async () => {
                                if (this.props.post.images.length > 0) {
                                    await this.props.setPostForOption(
                                        this.props.post
                                    );
                                    this._postPopup(true);
                                } else
                                    this.redirectToNextPage(this.props.post.id);
                            }}
                        >
                            ...
                            {post.sharePostForm ? (
                                ...
                                <div style={sharePostStyle}>
                                    <div className="header">
                                        ...
                                        <div
                                            className={
                                                "wrapper-detail margintop-header"
                                            }
                                        >
                                            <div
                                                className={"name"}
                                    ...
                                    <div className={"body"}>
                                        ...
                                        {post.sharePostForm.images.length >
                                        0 ? (
                                            <PostImage
