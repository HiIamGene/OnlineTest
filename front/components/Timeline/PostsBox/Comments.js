...
import { setPostComment, setCreateComment, setPostForOption, setCreatePostHasPost } from "../../../redux/actions/feedAction";
import axios from "axios";
import { API_FEED_COMMENT, API_FEED_POST, API_FEED_EMOTION } from "../../../constant/ENV";
...
class Comments extends Component {
    ...
    async seeMoreShow() {
        ...
        await axios
            .get(
                `${API_FEED_POST}/${this.props.getPostId}/comment?page=${page}&perPage=${perPage}&postId=${
            ...
            .then(res => {
                ...
                if (res.data.payload.payload.length > 0) {
                    if (this.props.post.totalComment < page * perPage) {
                        this.setState({
                            isShowSeeMore: false,
                            isSeeMoreLoading: false
                        });
    ...
    createCommentForm(comment, index) {
                            ...
                            <div className={"name"}>
                                ...
                                >
                                    {comment.user.name.concat(comment.user.surname).length > 30
                                        ? `${comment.user.name.concat(comment.user.surname).substring(0, 30)}...`
    ...
    render() {
        ...
        const commentGet =
            this.state.commentGet !== undefined
                ? this.state.commentGet.map((item, index) => this.createCommentForm(item, index)).reverse()
                : "";
        return (
            <div
                className={"box"}
...
export default compose(
    ...
    connect(
        mapStateToProps,
        { setPostComment, setCreateComment, setPostForOption }
    ),
    withRouter
)(Comments);
